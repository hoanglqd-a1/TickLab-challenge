...
class Var:
    def __init__(self, value, children=None, op=None, local_gradients=None, require_grad=False, gradient=None):
        ...
        self.gradient = gradient if gradient is not None else np.zeros_like(self.value).astype('float64')
    ...
    def backward(self, grad=1.0):
        if not self.require_grad:
            return
        grad = np.array(grad).astype('float64')
        ...
        if self.__children is not None:
            for child, local_grad in zip(self.__children, self.local_gradients):
...
def exp(A):
    ...
    result = np.maximum(np.exp(A.value), 1e-6)
    return Var(result, children=[A], op='exp', local_gradients=[lambda x: x * result], require_grad=A.require_grad)
def log(A):
    A = A if isinstance(A, Var) else Var(A)
...
def maximum(A, B):
    ...
    return Var(np.maximum(A.value, B.value), children=[A, B],
                ...
                require_grad=A.require_grad or B.require_grad)
def max(A, axis=None):
    A = A if isinstance(A, Var) else Var(A)
...
def conv2d(input: Var, kernel: Var, bias:Var, padding, stride):
    ...
    _, padding_h, padding_w, _ = padding_input.shape
    for i in range(0, padding_h - ksize + 1, stride):
        for j in range(0, padding_w - ksize + 1, stride):
...
def conv2d_input_backprop(gradient, padding_a, kernel, stride, padding):
    ...
    batch_size,m,n,_ = padding_a.shape
...
def maxpool2d_backprop(gradient, input: np.ndarray, output_shape, pool_size, stride):
    ...
    batch_size, height, width, depth = output_shape
    for i in range(height):
        for j in range(width):
            ...
            max_position = np.where(np.abs(a - mx) < 1e-8, 1, 0)
            local_gradient[:,i*stride:i*stride+pool_size,j*stride:j*stride+pool_size,:] = (max_position.transpose(1,2,0,3) * gradient[:,i,j,:]).transpose(2,0,1,3)
