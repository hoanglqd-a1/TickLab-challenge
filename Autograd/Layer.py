...
class Layer(BaseLayer):
    def __init__(self, shape, activation, prev_layer):
        self.activation = activation
        w = ag.Var(np.random.rand(shape, prev_layer.shape) * np.sqrt(1/prev_layer.shape), require_grad=True)
        b = ag.Var(np.random.rand(shape) * np.sqrt(1/prev_layer.shape), require_grad=True)
    ...
    def __activate(self):
        if self.activation == 'ReLU':
            ...
            tmp = self.z.transpose(1,0)
            z = (tmp-mx).transpose(1,0)
