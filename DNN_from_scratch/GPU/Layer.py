...
class Layer(BaseLayer):
    def __init__(self, shape, activation, prev_layer):
        self.activation = activation
        w = cp.random.rand(shape, prev_layer.shape) * cp.sqrt(2/prev_layer.shape)
        b = cp.random.rand(shape) * cp.sqrt(2/prev_layer.shape)
    ...
    def __activate(self):
        ...
        self.a = cp.asnumpy(self.a.astype('float32'))
...
class Conv2D(BaseLayer):
    ...
    def feed_forward(self):
        ...
        tmp_padding_a = cp.asarray(np.expand_dims(self.padding_a, axis=1))
        tmp_padding_a = cp.repeat(tmp_padding_a, self.filters, axis=1)
        w = cp.asarray(self.w)
        b = cp.asarray(self.b)
        z = cp.zeros(shape=self.z.shape)
        for i in range(0, m - kernel + 1, self.stride):
            for j in range(0, n - kernel + 1, self.stride):
                tmp = tmp_padding_a[:,:,i:i+kernel,j:j+kernel,:]
                ii = i//self.stride
                jj = j//self.stride
                z[:,ii,jj,:] = cp.sum((tmp * w), axis=(2,3,4)) + b
    ...
    def back_propagation(self, e):
        ...
        tmp_padding_a = cp.asarray(tmp_padding_a)
        for i in range(0, m - kernel + 1, self.stride):
            for j in range(0, n - kernel + 1, self.stride):
                tmp = tmp_padding_a[:,i:i+kernel,j:j+kernel,:,:]
                ii  = i//self.stride
        ...
        tmp_kernel = cp.asarray(tmp_kernel)
        for i in range(0, m - kernel + 1, self.stride):
            for j in range(0, n - kernel + 1, self.stride):
                ii  = i//self.stride
                jj  = j//self.stride
