...
class Adam(Optimizer):
    ...
    def compute_change(self,d_W,d_b):
        ...
        self.v_w = self.b2 * self.v_w + (1 - self.b2) * cp.square(d_W)
        self.b1_t *= self.b1
        self.b2_t *= self.b2
        m_w_hat  = self.m_w / (1 - self.b1_t)
        v_w_hat  = self.v_w / (1 - self.b2_t)
        self.m_b = self.b1 * self.m_b + (1 - self.b1) * d_b
        self.v_b = self.b2 * self.v_b + (1 - self.b2) * cp.square(d_b)
        m_b_hat  = self.m_b / (1 - self.b1_t)
        v_b_hat  = self.v_b / (1 - self.b2_t)
        change_W = self.learning_rate * m_w_hat / (cp.sqrt(v_w_hat) + 1e-8)
        change_b = self.learning_rate * m_b_hat / (cp.sqrt(v_b_hat) + 1e-8)
