...
import cupy_DNN.Loss as Loss, cupy_DNN.Layer as Layer, cupy_DNN.Optimizers as Optimizers
            ...
            if verbose == 1:
                if epochs is None:
                    if (iter + 1) % 10 == 0:
                        ...
                        print(f'Iter: {iter}')
                        cost = self.loss.cost(y_predict, y_train)
                        accuracy = self.loss.accuracy(y_predict, y_train)
                        out = "Training. Cost: {:.4f}; Accuracy: {:.4f}".format(cost, accuracy)
                        train_cost.append(cost)
                        train_accuracy.append(accuracy)
                        if x_validate is not None:
                            y_predict = self.predict(x_validate)
                            cost = self.loss.cost(y_predict, y_validate)
                            accuracy = self.loss.accuracy(y_predict, y_validate)
                            validate_cost.append(cost)
                            validate_accuracy.append(accuracy)
                            out += "; Validation: Cost: {:.4f}; Accuracy: {:.4f}".format(cost, accuracy)
                        print(out)
                ...
                pass
            print("iter:", iter)
        return [train_cost, train_accuracy, validate_cost, validate_accuracy]

    def predict(self, X_test, batch_size=None):
        if batch_size == None:
            batch_size = min(512, X_test.shape[0])
...
def main():
    ...
    a, b, c, d = model.fit(x_train, y_train, x_validate, y_validate, batch_size=32, epochs=1)
    print(a, b, c, d)
